...
> ESP is a new architectural design pattern for managing the 'M' in your MVC application. Based on reactive programming and domain driven design, ESP is based around a core model that comprehensively captures your entire application's state and is published to your controllers as single observable stream of changes. Inside the the model, ESP events and processors provide a scalable pattern for writing and refactoring complicated interdependent logic. It's great for complex applications with complex state transitions and real-time events (e.g. streaming data).
